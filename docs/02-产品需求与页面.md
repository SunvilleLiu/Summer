...
#### 2.4.2 Organization、LegalEntity 与企业认领
...
本卷要求以下对象至少具备版本化状态机并覆盖下列最低状态集合；完整状态机、迁移守卫与终态语义由系统设计权威定义，本表只提出需求下限：

| 需求 ID | 对象 | 最低状态集合 | 关键要求 |
|---|---|---|---|
| FR-ORG-013 | Organization | DRAFT/VERIFYING/ACTIVE/SUSPENDED/CLOSED | CLOSED 前受控终止活动订阅、撤销 capability 与 owner connector authorization；其作为 ACTIVE PRIMARY_PROVIDER/PRIMARY_CLIENT 的 Workspace 必须先关闭，或完成获批 successor/handover 且旧 Workspace 收口，主 participant 不得直接 LEFT，PARTNER 才可按影响评估单独 LEFT；再结束未来 membership/grant/book assignment并处置资金/争议/外部 UNKNOWN；任何无法处置项阻断关闭；LegalEntity、授权版本与历史只读保留 |
| FR-ORG-014 | Organization claim | DRAFT/REVIEWING/ACTIVE/SUSPENDED/REJECTED/REVOKED | ACTIVE 前不得建立企业管理权 |
| FR-WSP-016 | Workspace | DRAFT/ACTIVE/SUSPENDED/CLOSING/CLOSED | ACTIVE 需要双主 participant；关闭前先评估 |
| FR-WSP-017 | workspace_participant | INVITED/ACTIVE/SUSPENDED/LEFT | 角色与能力匹配；LEFT 保留历史 |
| FR-CTR-005 | 合同 | DRAFT/PENDING_APPROVAL/APPROVED/REJECTED/SIGNING/SIGNED/SIGN_FAILED/EFFECTIVE/FULFILLED/TERMINATED/EXPIRED | 审批/签署后正文冻结；变更建立版本/补充协议 |
| FR-SVC-009 | 服务项目 | PENDING/CONFIGURING/PENDING_ACTIVATION/ACTIVE/PAUSED/COMPLETED/EXPIRED/TERMINATED | 法定义务不随普通暂停自动取消 |
| FR-ACC-014 | AccountingBook | DRAFT/INITIALIZING/INITIALIZATION_FAILED/ACTIVE/SUSPENDED/TERMINATED/ARCHIVED | 唯一 owner 为 `owner_legal_entity_id`，Organization 从 LegalEntity 派生；受托作业另校验有效 `workspace_book_assignment`；归档不删历史 |
| FR-ACC-015 | 会计期间与任务 | UNOPENED/COLLECTING/BOOKKEEPING/REVIEWING/READY_TO_CLOSE/CLOSED | 关闭版本不可普通写；反结账生成后继期间版本 |
| FR-TAX-013 | 税费测算 | DRAFT/CALCULATED/REVIEWING/CONFIRMING/CONFIRMED/SUPERSEDED | 内容变化建立后继测算，不覆盖已确认版本 |
| FR-TAX-014 | 申报 | DRAFT/REVIEWING/READY_TO_SUBMIT/SUBMITTING/UNKNOWN/ACCEPTED/REJECTED/CORRECTED/VOIDED | UNKNOWN 只查单；更正/作废保留原链 |
| FR-COL-008 | 材料与外勤 | 材料需求 WAITING/SUBMITTED/UNDER_REVIEW/ACCEPTED/REJECTED/WAIVED/EXPIRED；材料版本 DRAFT/SUBMITTED/ACTIVE/REJECTED/SUPERSEDED/REVOKED/EXPIRED；外勤报告 DRAFT/SUBMITTED/ACCEPTED/REJECTED/CORRECTED |已提交/生效版本不可覆盖，纠错建立后继 |
| FR-COL-009 | 确认请求 | DRAFT/SENT/OPENED/ACKNOWLEDGED/CONFIRMED/DISPUTED/EXPIRED/CANCELLED/SUPERSEDED | 内容变化使旧请求 SUPERSEDED；终态幂等 |
| FR-PLT-006 | 订阅 | DRAFT/TRIAL/ACTIVE/GRACE/READ_ONLY/SUSPENDED/EXPIRED/TERMINATED | 欠费传播不得删除历史或隐藏法定义务；终止不删除历史 |
...
#### 3.2.3 客户企业门户 `ENTERPRISE_PORTAL`
...
### 3.6 服务机构工作端：公共、Organization 与 Workspace
