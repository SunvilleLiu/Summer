...
### 4. Organization 与身份模型

#### 4.1 Organization 不设永久类型
...
16. `SOD-CONFLICT-001`：通用职责冲突守卫：同一自然人不得在同一业务对象上同时承担互斥职责（申请/审批/执行、编制/复核、提交/决定等）；具体互斥组合以本表专项规则和已发布审批模板为准，命中即失败关闭并返回职责分离错误
