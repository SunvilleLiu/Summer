...
### 1.5 LegalEntity 解析与企业认领

#### 1.5.1 PROVIDER 私有潜客边界
...
### 1.7 participant 与 membership

#### 1.7.1 participant 状态
...
### 1.8 WorkspaceSubject、账套与服务权

#### 1.8.1 workspace_subject
...
本文定义领域边界、聚合、不变量、跨域流程、事务和事件。字段、索引与分区以 本卷《数据模型与字段字典》为准；接口与错误码以 本卷《API、事件与错误码》为准；角色审批以 本卷《身份、权限与审批》和 本卷附件《权限审批种子》为准。
...
#### 2.7.2 OperatingFinance：回款、退款与开票
...
### 2.11 OperatingFinance 与 InternalManagement

#### 2.11.1 OperatingFinance
...
### 2.13 EnterpriseCollaboration 领域
...
#### 3.2.1 Organization 状态 `STATE-ORG-001`
...
#### 3.2.1.1 LegalEntity claim `STATE-ORG-002`
...
#### 3.2.2 Workspace 状态 `STATE-WSP-001`
...
#### 3.2.3 Workspace 参与关系 `STATE-WSP-002`
...
#### 3.2.4 Workspace 成员 `STATE-WSP-003`
...
#### 3.4.2 Organization 客户视图和联系人 `STATE-CRM-002`
...
#### 3.5.3 回款与核销 `STATE-FIN-001`、`CALC-FIN-001`
...
#### 3.5.4 退款 `STATE-FIN-002`、`CALC-FIN-002`
...
#### 3.5.5 服务机构销售开票 `STATE-FIN-003`、`CALC-FIN-003`
...
#### 3.6.1 合同服务项和服务项目 `STATE-SVC-001`
...
#### 3.7.1 AccountingBook `STATE-ACC-001`
...
#### 3.7.5 期初、账簿和结账 `STATE-ACC-005`、`CALC-ACC-002`
...
#### 3.8.1 税费测算 `STATE-TAX-001`、`CALC-TAX-001`
...
#### 3.8.3 缴退税 `STATE-TAX-003`、`CALC-TAX-002`
...
#### 3.8.4 个税、社保、公积金和年度事项 `STATE-TAX-004`
...
#### 3.9.2 内部工资和报销 `STATE-INT-002`、`CALC-INT-001`
...
#### 3.11.1 供应商、凭证、Organization 授权和 Workspace grant
...
#### 3.11.2 请求 `STATE-CONN-002` 与事件 `STATE-CONN-006`
...
#### 5.2.3 user_mfa_factor / auth_challenge
...
#### 5.2.5 workspace_session_context
...
#### 5.3.5 organization_legal_entity_claim
...
#### 5.3.6 provider_customer_profile
...
#### 5.3.7 organization_relationship（`ENT-ORG-006`）
...
#### 5.4.5 workspace_book_assignment
...
#### 5.4.6 workspace_lifecycle_case（`ENT-WSP-006`）
...
#### 5.4.7 workspace_lifecycle_impact（`ENT-WSP-007`）
...
#### 5.4.8 workspace_event（`ENT-WSP-008`）
...
#### 5.4.10 published_projection（`ENT-WSP-010`）
...
#### 5.5.1 permission / role / role_permission
...
#### 5.5.3 field_access_policy / permission_policy / sod_policy
...
#### 5.6.1 approval_template / version / node
...
#### 5.6.2 approval_instance / task / decision_event
...
#### 5.6.3 audit_event / outbox_event / job_task
...
### 5.8 Contract 与 OperatingFinance 数据
...
#### 5.10.1 accounting_book / book_assignment / period
...
### 5.12 InternalManagement 与企业协作

#### 5.12.1 provider 内部管理
...
#### 5.12.2 enterprise_collaboration
...
#### 5.12.3 support_access_request / grant / context（`ENT-SUP-001`）
...
#### 5.13.1 connector_provider / adapter / platform_credential

平台表保存供应商、能力类型、环境、适配器制品/Schema 摘要、非密钥端点和 credential mode。平台凭证按环境/版本保存密钥服务引用；密钥/证书不回显。

#### 5.13.2 organization_connector_authorization
...
#### 5.13.3 workspace_connector_grant
...
#### 5.13.4 connector_request / attempt / event / usage
...
#### 6.8.4 CommercialBilling 与资金安全（`DOM-BILL-001`）
...
### 7.3 Organization 与主体认领

#### 7.3.1 Organization 能力
...
##### 8.5.6.1 CommercialBilling 逐操作冻结

##### 8.5.6.2 IAM、Internal 与 Support 映射
...
##### 8.5.6.4 Organization/Workspace 生命周期与关闭传播映射
...
#### 8.8.1 身份、Organization 与 Workspace
...
#### 8.10.3 CommercialBilling、合同编号与 H5 专项错误
